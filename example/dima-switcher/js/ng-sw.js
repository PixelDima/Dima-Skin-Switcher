...
+(function () {
    ...
    var app = angular.module("sw-switcher", ["ipCookie"]);
    ...
    /*================Cookie Controller===================*/
    app.controller("CookieController", function ($scope, $rootScope, ipCookie) {

        var isBoxed   = ipCookie("boxVal"),
            isPatt    = ipCookie("pattCooki"),
            isImg     = ipCookie("imgCooki"),
            isStyle   = ipCookie("styleCooki"),
            isColored = ipCookie("coloCokies");
        ...
        if (isPatt) {
            $("body").css({
                "background": "url("+pluginsUrl+"/patterns/" + isPatt + ") top left repeat fixed"
            });
        }
        /**
        ...
        if (isImg) {
            $("body").css({
                "background": "url("+pluginsUrl+"/bg/" + isImg + ") top left fixed"
            });
        ...
        if (isStyle) {
            ...
            ipCookie.remove("isStyle", { path: pluginsUrl });
        }
        /**
        ...
        $scope.boxed = function () {
            ...
            ipCookie("boxVal", $rootScope.boxedval, { path: pluginsUrl });
        };
        /**
    ...
    app.controller("optionsController", function ($scope, $rootScope, ipCookie, $timeout) {
        ...
        $scope.savaPattern = function (pattern) {
            ...
            $(patternsInto).css({
                "background": "url("+pluginsUrl+"/patterns/" + pattern + ") top left repeat fixed"
            });
            ipCookie("pattCooki", $rootScope.pattCooki , { path: pluginsUrl });
            ipCookie.remove("imgCooki", { path: pluginsUrl });
        };
        ...
        $scope.savaImage = function (image) {
            ...
            $(this).parent().siblings().removeClass("active").end().addClass("active");
            $(pgInto).css({
                "background": "url("+pluginsUrl+"/bg/" + image + ") top left fixed"
            });
            $(pgInto).addClass("sw-background");

            ipCookie("imgCooki", $rootScope.imgCooki, { path: pluginsUrl });
            ipCookie.remove("pattCooki", { path: pluginsUrl });
        };
        ...
        $scope.savaStyle = function (style) {
            ...
            ipCookie("styleCooki", $rootScope.styleCooki, { path: pluginsUrl }); //save cookies
        };
        ...
        $scope.saveColors = function () {
            ...
            ipCookie("coloCokies", $rootScope.coloCokies, { path: pluginsUrl });
        };
        /**
        ...
        $scope.reset = function () {
            ...
            ipCookie.remove("imgCooki", { path: pluginsUrl });
            ipCookie.remove("pattCooki", { path: pluginsUrl });
            ipCookie.remove("styleCooki", { path: pluginsUrl });
            ipCookie.remove("boxVal", { path: pluginsUrl });
            ipCookie.remove("patttCooki", { path: pluginsUrl });
            ipCookie.remove("coloCokies", { path: pluginsUrl });
            //refresh a page
            location.reload();
        };
        /**
    ...
    $(document).ready(function () {
        ...
        function dima_switcher() {
            ...
            $(".sw-patterns a").each(function () {
                ...
                if (patt) {
                    $(this).css({
                        "background": "url("+pluginsUrl+"/patterns/" + patt + ")"
                    });
                }
            });
            ...
            $(".sw-images a").each(function () {
                ...
                if (image) {
                    if (image) {
                        $(this).append("<img src="+pluginsUrl+"/bg/small-" + image + " alt=\"\">");
                    }
                }
            });
            /**
